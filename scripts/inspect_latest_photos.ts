...
async function inspectPhotos() {
  ...
  photos.forEach((p, i) => {
    ...
    console.log(`Filename: ${p.metadata?.filename}`);
    console.log(`Description: "${p.metadata?.description}"`);
    console.log(`Tags: ${JSON.stringify(p.metadata?.tags)}`);
    // Log full metadata keys to see if we missed anything
    console.log(`All Metadata Keys: ${Object.keys(p.metadata || {})}`);
  });
